...
export default {
  ...
  methods: {
    async init() {
      ...
      try {
        this.entities = await this.$api.director.getAll();
      } catch (error) {
    ...
    async save() {
      ...
      try {
        if (this.action === "CREATE") {
          let { id } = await this.$api.director.add(this.entity);
          this.entity.id = id;
    ...
    async savePassword() {
      ...
      this.loading_btn = true;
      try {
        await updatePasswordByAdmin(this.entity.id, this.new_password);
    ...
    async saveAll() {
      ...
      try {
        ...
        let { success, errors } = await this.$api.director.createMassive({
          users,
        });
        await this.init();
