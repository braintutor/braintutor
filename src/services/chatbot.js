  ...
  train(knowledge, entities) {
    ...
    this.knowledge.forEach(({ questions }, idx) => {
      this.x = this.x.concat(
        questions.map((question) => this._getHash(question))
      );
  ...
  setEntities(answers) {
    return answers.map((answer) => {
      ...
      if (required_entities)
        required_entities.forEach((required_entity) => {
          // (1) @user@first_name  (2) @user@last_name
          let [object, attribute] = required_entity.split("@").slice(1, 3); // (1) [user, first_name]  (2) [user, last_name]
          answer = answer.replace(
            required_entities,
            this.entities[object][attribute]
          ); // (1) Hi Mitsuo @user@last_name.  (2) Hi Mitsuo Yshara.
        });
      return answer;
    });
  }
